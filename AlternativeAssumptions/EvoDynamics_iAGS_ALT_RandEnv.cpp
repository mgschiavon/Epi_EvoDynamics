...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Random environment
//
...
// (PAR_E). The environment fluctuates randomly with frequency (nu) between two 
...
// output file.
// NOTE: A variant from EvoDynamics_ALT_RandEnv.cpp that prints only AGS file and 
...
int main(int argn, char *args[])
{
	...
	int iS = atof(args[10]);	// Replica index (i.e. value from seeds[] to use to generate random numbers).
		...
		int E = E0;				// Initialize environment.
		...
		for (int iG=1; iG<=GMAX; iG++)
		{
			...
			// Environment fluctuates randomly with frequency nu:
			if (ran1(&seed) <= nu)
			{
				E = (E+1)%2;
				myPRINTflag = 1;
