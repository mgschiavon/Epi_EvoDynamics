...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Protein distribution as phenotype
//
...
// defined in the GeneNetwork class (see GeneNetwork_ALT_Phen_PD.h). The population 
...
// algorithm (alg: strictly 1, Gillespie, in this case), as well as the initial 
...
// output file.
// NOTE: A variant from EvoDynamics_ALT_Phen_PD.cpp that prints only AGS file and 
...
#include <time.h>
#include <cmath>
#include "GeneNetwork_ALT_Phen_PD.h"
...
int main(int argn, char *args[])
{
	...
	bool alg = 1;			// Type of algorithm to simulate with (strictly 1 = Gillespie).
	...
	int iS = atof(args[10]);	// Replica index (i.e. value from seeds[] to use to generate random numbers).
		...
		for (int iG=1; iG<=GMAX; iG++)
		{
			...
			for (int i = 0; i<N; i++)
			{
				...
				{
					myPop[i].updateTime(&seed,E, AOPT, V);
					myPop[i].updateSystem(&seed);
				}
				myPop[i].w = myPop[i].w/myPop[i].time;
