...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Normal Fitness function
//
...
// and the fitness in each environment is calculated using Normal function 
// centred in the optimal protein number and variance V*AOPT. The cells to be 
...
// NOTE: A variant from EvoDynamics_ALT_WFunct_Normal.cpp where the seed index 
//       needs to be specified as input by the user (from 0 to 9).
	...
	int iS = atof(args[10]);	// Replica index (i.e. value from seeds[] to use to generate random numbers).
		...
		for (int iG=1; iG<=GMAX; iG++)
		{
			...
			for (int i = 0; i<N; i++)
			{
				...
				myPop[i].calculateFitnessNORMAL(E, AOPT, V);
