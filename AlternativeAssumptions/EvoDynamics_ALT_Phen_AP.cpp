...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Average protein as phenotype
//
...
// defined in the GeneNetwork class (see GeneNetwork_ALT_Phen_AP.h). The population 
...
#include <time.h>
#include <cmath>
#include "GeneNetwork_ALT_Phen_AP.h"
	...
	for(int iS=0; iS<numRep; iS++)
	{
		...
		for (int iG=1; iG<=GMAX; iG++)
		{
			...
			for (int i = 0; i<N; i++)
			{
				...
				myPop[i].mA = myPop[i].mA/myPop[i].time;
