...
//	EVOLUTIONARY MODEL USING AVERAGE PROTEIN AS PHENOTYPE
//
...
class GeneNetwork
{
	public:
		...
		// Phenotype:
		double A, mA;				// Protein & life-average protein number
	...
	{
		A = A0;		// Protein number
		mA = A0;	// Life-average protein number
	...
	{
		w = 1/(1+((mA-Aopt[myE])*(mA-Aopt[myE])/(v*Aopt[myE])));	// Lorentzian fitness function.
	}
