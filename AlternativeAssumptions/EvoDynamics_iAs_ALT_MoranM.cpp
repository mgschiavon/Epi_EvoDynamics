...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Moran model
//
...
// defined in the GeneNetwork class (see GeneNetwork_ALT_MoranM.h). The 
// population size (N), environmental fluctuation frequency (nu), selection 
// pressure (sT), mutation frequency (u), mutation step size (M), gene 
// expression dynamics' algorithm (alg: 0, Deterministic; 1, Gillespie), as 
// well as the initial genotype (k0; nH0; KD0) are input arguments defined by 
// the user. The simulation runs for GMAX generations. Using Moran model, 
// reproduction and death events are treated as stochastic events allowing 
// overlapping generations. At each time step, an individual is chosen for 
// reproduction using tournament selection, and an individual is randomly chosen 
// from the population for death to keep the population size N fixed. N time steps 
// occur in the "expected life time" [tLIFE]. The same initial protein number (A0) 
// is assumed for all cells in the population. Some parameters in the gene network 
// model are assumed to be fixed: the basal synthesis activity (ALPHA) and the 
// protein degradation rate (DEG). From the three parameters that define the 
// cell's genotype (k; nH; KD), which ones would actually allow to evolve is 
// determined by the parE class (PAR_E). The environment fluctuates with a fixed 
// frequency (nu) between two states (0, High; 1, Low), and the initial 
// environmental state is defined by E0. Each environment is characterized by an 
// optimal protein number (AOPT); and the fitness in each environment is 
// calculated using a Lorentzian function centered in the optimal protein number 
// and a width measure V. The cells to be cloned in the next generations are 
// selected applying the tournament selection scheme, with a probability u of 
// randomly mutate its parameter values. Every generation, the average 
// genotype & subpopulations is printed in the AGS output file; every cycle, 
// the adaptation strategy per cycle is printed in the 
...
// NOTE: A variant from EvoDynamics_ALT_MoranM.cpp where the seed index needs to be 
...
#include <time.h>
#include <cmath>
#include "GeneNetwork_ALT_MoranM.h"
...
int main(int argn, char *args[])
{
	...
	double rLE = N/tLIFE;		// Rate of life events.
		...
		double tWait;		// Waiting time between life events.
		double tGen = 0;	// Exact time per "generation" to properly normalyze generation metrics.
		double tCycle = 0;	// Exact time per "cycle" to properly normalyze cycle metrics.
        while (t < (tLIFE*GMAX))
		{
			// Define time of the next life event:
			tWait = -log(1-(double)ran1(&seed))/rLE;	// Time for the next reaction
			t += tWait;										// Actualize global time
			
			// Environment fluctuates with frequency tLIFE/nu (0 -> E_high & 1 -> E_low).
			E = int((int(t)%(2*int(tLIFE/nu)))/int(tLIFE/nu))%2;
			...
			for (int i = 0; i<N; i++)
			{
				...
				{
					myPop[i].LBis = 0;
					myPopM.k += (myPop[i].k*tWait);
					myPopM.nH += (myPop[i].nH*tWait);
					myPopM.KD += (myPop[i].KD*tWait);
					myPopM.A += (myPop[i].A*tWait);
					myPopM.w += (myPop[i].w*tWait);
					myPopM.f += tWait;
				}
				// BISTABLE
				else
				{
					myPopB.k += (myPop[i].k*tWait);
					myPopB.nH += (myPop[i].nH*tWait);
					myPopB.KD += (myPop[i].KD*tWait);
					myPopB.A += (myPop[i].A*tWait);
					myPopB.w += (myPop[i].w*tWait);
					myPopB.f += tWait;
				}
			}
			
			// Calculate and print population data every tLIFE time:
			if(int(t/tLIFE) > int(tGen/tLIFE))
			{
				// PRINTING - Average genotype & subpopulations:
				myPopB.normalize(N);
				myPopM.normalize(N);
				AGS << int(t/tLIFE) << '\t' << E << '\t' << myPopB.f/(t-tGen) << '\t';
				...
				if((int(t/tLIFE)>(GMAX-(Ep/nu))) && (int(t/tLIFE)%int(1/nu)<10))
				{
					for (int i = 0; i<N; i++)
					{
						IxE << int(t/tLIFE) << '\t' << E << '\t' << i << '\t';
				...
				if(int(t/tLIFE)%int(2/nu)==0)
				{
					...
					for (int i=0; i<N; i++)
					{
						...
						if(int(t/tLIFE)>(GMAX-(Cp*2/nu)))
						{
							// PRINTING - Lineage information x cycle:
							LxC << int(t/tLIFE) << '\t';
							LxC << myPop[i].LTag << '\t' << myPop[i].LMut << '\t' << myPop[i].LBis << '\t';
							LxC << myPop[i].LFit/tCycle << '\t';
			...
			selectionTournamentMM(myPop, N, sT, u, M, &seed, PAR_E);
		}
