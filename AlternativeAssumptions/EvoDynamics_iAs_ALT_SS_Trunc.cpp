...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Truncation selection
//
...
// defined in the GeneNetwork class (see GeneNetwork_ALT_SS.h). The population size 
...
// cloned in the next generations are selected applying the truncation selection 
...
// NOTE: A variant from EvoDynamics_ALT_SS_Trunc.cpp where the seed index needs 
...
int main(int argn, char *args[])
{
	...
	double sT = atof(args[3]);	// Selection strength.
	...
	int iS = atof(args[10]);	// Replica index (i.e. value from seeds[] to use to generate random numbers).
		...
		sprintf(myFileName,"AGS_N%d_nu%1.4f_s%1.2f_u%1.3f_M%1.2f_a%d_k%1.3f_nH%1.2f_KD%1.2f_%d.dat",N,nu,sT,u,M,alg,k0,nH0,KD0,iS);
		...
		sprintf(myFileName,"ASC_N%d_nu%1.4f_s%1.2f_u%1.3f_M%1.2f_a%d_k%1.3f_nH%1.2f_KD%1.2f_%d.dat",N,nu,sT,u,M,alg,k0,nH0,KD0,iS);
		...
		sprintf(myFileName,"LxC_N%d_nu%1.4f_s%1.2f_u%1.3f_M%1.2f_a%d_k%1.3f_nH%1.2f_KD%1.2f_%d.dat",N,nu,sT,u,M,alg,k0,nH0,KD0,iS);
		...
		sprintf(myFileName,"IxE_N%d_nu%1.4f_s%1.2f_u%1.3f_M%1.2f_a%d_k%1.3f_nH%1.2f_KD%1.2f_%d.dat",N,nu,sT,u,M,alg,k0,nH0,KD0,iS);
		...
		for (int iG=1; iG<=GMAX; iG++)
		{
			...
			selectionTruncation(myPop, N, sT, u, M, &seed, PAR_E);
		}
