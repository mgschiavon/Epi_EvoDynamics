...
//	EVOLUTIONARY MODEL USING PROTEIN DISTRIBUTION AS PHENOTYPE
//
...
class GeneNetwork
{
	...
	void updateTime(long *seed,int myE, double Aopt[2], double v)
	{
		...
		double tempW = 1/(1+((A-Aopt[myE])*(A-Aopt[myE])/(v*Aopt[myE]))); // Lorentzian fitness function
		w += (tempW*re_time);					// Update average fitness
	}
