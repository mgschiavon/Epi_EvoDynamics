...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Proportional selection
//
...
// (N), environmental fluctuation frequency (nu), 
...
// cloned in the next generations are selected applying the proportional selection 
...
// output file.
// NOTE: A variant from EvoDynamics_ALT_SS_Prop.cpp that prints only AGS file and 
...
int main(int argn, char *args[])
{
	...
	int sT = 0;					// Selection strength cannot be tuned in this selection scheme.
	...
	int iS = atof(args[10]);	// Replica index (i.e. value from seeds[] to use to generate random numbers).
		...
		for (int iG=1; iG<=GMAX; iG++)
		{
			...
			selectionProportional(myPop, N, u, M, &seed, PAR_E);
		}
