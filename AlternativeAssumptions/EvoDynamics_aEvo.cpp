...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Basal activity evolving
//
...
// defined in the GeneNetwork class (see GeneNetwork_aEvo.h). The population size 
...
// the population. Only the protein degradation rate (DEG) is assumed to be fixed. 
// From the four parameters that define the cell's genotype (k; nH; KD; alpha), 
...
int main(int argn, char *args[])
{
	...
	PAR_E.a = 1;			// Basal synthesis activity (a=1 -> No feedback)
	...
	if (argn < 11)
	{
		cerr << "Error! Input nine arguments: N; nu; sT; u; M; Algorithm: (0) Deterministic, (1) Gillespie; k0; nH0; KD0; a0." << endl;
		exit(0);
	}
	...
	double a0 = atof(args[10]);	// Initial value for the basal synthesis activity (a).
	...
	for(int iS=0; iS<numRep; iS++)
	{
		...
		sprintf(myFileName,"AGS_N%d_nu%1.4f_s%d_u%1.3f_M%1.2f_a%d_k%1.3f_nH%1.2f_KD%1.2f_a%1.2f_%d.dat",N,nu,sT,u,M,alg,k0,nH0,KD0,a0,iS);
		ofstream AGS(myFileName,ios::out);
		AGS << "Generation" << '\t' << "Environment" << '\t' << "Bis-Fraction" << '\t';
		AGS << "<k>_B" << '\t' << "<nH>_B" << '\t' << "<KD>_B" << '\t' << "<a>_B" << '\t';
		AGS << "<A>_B" << '\t' << "<w>_B" << '\t';
		AGS << "<k>_M" << '\t' << "<nH>_M" << '\t' << "<KD>_M" << '\t' << "<a>_M" << '\t';
		AGS << "<A>_M" << '\t' << "<w>_M" << endl;
		// Adaptation strategy x cycle:
		sprintf(myFileName,"ASC_N%d_nu%1.4f_s%d_u%1.3f_M%1.2f_a%d_k%1.3f_nH%1.2f_KD%1.2f_a%1.2f_%d.dat",N,nu,sT,u,M,alg,k0,nH0,KD0,a0,iS);
		...
		sprintf(myFileName,"LxC_N%d_nu%1.4f_s%d_u%1.3f_M%1.2f_a%d_k%1.3f_nH%1.2f_KD%1.2f_a%1.2f_%d.dat",N,nu,sT,u,M,alg,k0,nH0,KD0,a0,iS);
		ofstream LxC(myFileName,ios::out);
		LxC << "Generation" << '\t';
		LxC << "LTag" << '\t' << "LMut" << '\t' << "LBis" << '\t' << "LFit" << '\t';
		LxC << "k_H" << '\t' << "nH_H" << '\t' << "KD_H" << '\t' << "a_H" << '\t';
		LxC << "k_L" << '\t' << "nH_L" << '\t' << "KD_L" << '\t' << "a_L" << endl;
		// Individuals information per epoch (first 9 & last generations):
		sprintf(myFileName,"IxE_N%d_nu%1.4f_s%d_u%1.3f_M%1.2f_a%d_k%1.3f_nH%1.2f_KD%1.2f_a%1.2f_%d.dat",N,nu,sT,u,M,alg,k0,nH0,KD0,a0,iS);
		...
		myPop[0].ic(A0,k0,a0,nH0,KD0,DEG);
		myPop[0].LTag = 0;
		for (int i=1; i<N; i++)
		{
			myPop[i] = myPop[0];
		...
		for (int iG=1; iG<=GMAX; iG++)
		{
			...
			for (int i = 0; i<N; i++)
			{
				...
				{
					IxE << iG << '\t' << E << '\t' << i << '\t';
					IxE << myPop[i].k << '\t' << myPop[i].nH << '\t' << myPop[i].KD << '\t' << myPop[i].a << '\t';
			...
			myPopM.normalize(N);
			AGS << iG << '\t' << E << '\t' << myPopB.f << '\t';
			AGS << myPopB.k << '\t' << myPopB.nH << '\t' << myPopB.KD << '\t' << myPopB.a << '\t';
			AGS << myPopB.A << '\t' << myPopB.w << '\t';
			AGS << myPopM.k << '\t' << myPopM.nH << '\t' << myPopM.KD << '\t' << myPopM.a << '\t';
			...
			if(iG%int(2/nu)==0)
			{
				...
				for (int i=0; i<N; i++)
				{
					...
					if(iG>(GMAX-(Cp*2/nu)))
					{
						...
						LxC << myPop[i].kE[0] << '\t' << myPop[i].nHE[0] << '\t' << myPop[i].KDE[0] << '\t' << myPop[i].aE[0] << '\t';
						LxC << myPop[i].kE[1] << '\t' << myPop[i].nHE[1] << '\t' << myPop[i].KDE[1] << '\t' << myPop[i].aE[1] << endl;
					}
