...
// SIMULATIONS - ALTERNATIVE ASSUMPTIONS: Weighted selection
//
...
// cloned in the next generations are selected applying the weighted selection 
...
// output file.
// NOTE: A variant from EvoDynamics_ALT_SS_Weight.cpp that prints only AGS file and 
...
int main(int argn, char *args[])
{
	...
	int iS = atof(args[10]);	// Replica index (i.e. value from seeds[] to use to generate random numbers).
		...
		for (int iG=1; iG<=GMAX; iG++)
		{
			...
			selectionWeighted(myPop, N, u, M, &seed, PAR_E);
		}
